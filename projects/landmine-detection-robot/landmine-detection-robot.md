...
*Fig 1 — GPS, GSM (via RS232) and the metal sensor feed the DSPIC30F4013; the RF link decodes drive commands into relay-switched motor control. Two 6V batteries supply a regulated +5V rail throughout.*
...
*Fig 2 — Each satellite's signal delay gives a range circle; where enough of them overlap pins down the robot's — and the mine's — position.*
...
| Component | Role |
|---|---|
| Chassis | Cast iron vehicle body, orange-painted |
| DSPIC30F4013 | Central microcontroller — sensing, GPS/GSM, motor control |
| Metal sensors ×2 | Inductive proximity — pins 10 & 38, blast + fragmentation coverage |
| GPS modem + antenna | Latitude/longitude fix — pins 25–26 via RS232 |
| GSM modem (SIM) | SMS location report — pins 27–28 via RS232 |
| RF TX/RX + encoder/decoder | 433.92 MHz remote drive link, ~100 ft range |
| DC gear motors ×2 | 60 RPM @ 12V, rear-wheel drive |
| Relays + relay driver | Switch motor direction from decoded RF commands |
| 2× 6V battery | Power for motors and regulated +5V logic supply |
| LCD screen | On-board display of GPS latitude/longitude |
| Wireless camera | Live view for remote navigation |
